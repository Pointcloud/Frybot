...
ManipulatorTeleop::ManipulatorTeleop()
    :node_handle_(""),
     priv_node_handle_("~"),
     with_gripper_(false)
{
  ...
  ROS_INFO("Manipulator initialization");
}
...
void ManipulatorTeleop::initClient()
{
...
void ManipulatorTeleop::initSubscriber()
{
  joint_states_sub_ = node_handle_.subscribe("joint_states", 10, &ManipulatorTeleop::jointStatesCallback, this);
  kinematics_pose_sub_ = node_handle_.subscribe("kinematics_pose", 10, &ManipulatorTeleop::kinematicsPoseCallback, this);
}

void ManipulatorTeleop::jointStatesCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
...
void ManipulatorTeleop::kinematicsPoseCallback(const open_manipulator_msgs::KinematicsPose::ConstPtr &msg)
{
...
std::vector<double> ManipulatorTeleop::getPresentJointAngle()
{
  return present_joint_angle_;
}
std::vector<double> ManipulatorTeleop::getPresentKinematicsPose()
{
  return present_kinematic_position_;
}

bool ManipulatorTeleop::setJointSpacePathFromPresent(std::vector<std::string> joint_name, std::vector<double> joint_angle, double path_time)
{
...
bool ManipulatorTeleop::setJointSpacePath(std::vector<std::string> joint_name, std::vector<double> joint_angle, double path_time)
{
  open_manipulator_msgs::SetJointPosition srv;
  srv.request.joint_position.joint_name = joint_name;
  srv.request.joint_position.position = joint_angle;
...
bool ManipulatorTeleop::setToolControl(std::vector<double> joint_angle)
{
  open_manipulator_msgs::SetJointPosition srv;
...
bool ManipulatorTeleop::setTaskSpacePathFromPresentPositionOnly(std::vector<double> kinematics_pose, double path_time)
{
...
void ManipulatorTeleop::printText()
{
  printf("\n");
  printf("---------------------------------\n");
  printf("Control Your Manipulator-PRO!\n");
  printf("---------------------------------\n");
...
void ManipulatorTeleop::setGoal(char ch)
{
...
void ManipulatorTeleop::restoreTerminalSettings(void)
{
    tcsetattr(0, TCSANOW, &oldt_);  /* Apply saved settings */
}

void ManipulatorTeleop::disableWaitingForEnter(void)
{
  struct termios newt;
...
int main(int argc, char **argv)
{
  ...
  ROS_INFO("Manipulator teleoperation using keyboard start");
  ManipulatorTeleop.disableWaitingForEnter();

  ros::spinOnce();
  ManipulatorTeleop.printText();
  ...
  while (ros::ok() && (ch = std::getchar()) != 'q')
  {
    ...
    ManipulatorTeleop.printText();
    ros::spinOnce();
    ManipulatorTeleop.setGoal(ch);
  }

  printf("input : q \tTeleop. is finished\n");
  ManipulatorTeleop.restoreTerminalSettings();
